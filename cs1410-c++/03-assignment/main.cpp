...
void push(Stack* stack, double num) {
  ...
  i->next = stack->top;
  // Then we want to update the stack so that the new item becomes the new
  // `top`.
  stack->top = i;
...
void printStack(const Stack* stack) {
  ...
  // Otherwise, for a non-empty stack, we'll grab the top item, just like
  // before:
  Item* i = stack->top;
  // And we'll output the value of the top item (and indicate that it's the
  // top):
